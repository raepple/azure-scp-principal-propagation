...
const { ConfirmPrompt, DialogSet, DialogTurnStatus, OAuthPrompt, TextPrompt, WaterfallDialog, DialogContainer } = require('botbuilder-dialogs');
...
class MainDialog extends LogoutDialog {
    ...
    async loginStep(stepContext) {
        ...
        if (tokenResponse) {
            return await stepContext.prompt(CONFIRM_PROMPT, 'Would you like to view your Azure AD access token?');
        }
    ...
    async displayTokenPhase2(stepContext) {
        ...
        if (aadTokenResponse) {
            await stepContext.context.sendActivity(`Here is your access token ${ aadTokenResponse.token }`);
        ...
        if (stepContext.result) {            
            ...
            if (aadTokenResponse && aadTokenResponse.token) {
                ...
                if (numberOfProducts > 0) {
                    ...
                    await stepContext.context.sendActivity("No products found. Missing authorizations?");
                }
            } else {
